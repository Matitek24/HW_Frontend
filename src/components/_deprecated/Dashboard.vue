  ...
  <div class="dashboard-wrapper" style="overflow: hidden;">
    ...
    <div class="container-fluid px-xxl-5 position-relative z-1">
      ...
      <div class="glass-panel p-4 p-lg-5">
        ...
        <SearchFilter :results-count="totalProjects" @search="handleSearch" />
          ...
          <table class="table align-middle custom-table" :class="{ 'table-hover': !isMobile }">
                ...
                <th class="ps-4 clickable-header" @click="sort('date')">
                  <div class="d-flex align-items-center gap-1">
                    ...
                    <span class="sort-icon" :class="{ active: currentSort === 'date' }">
                      <svg v-if="currentSort === 'date' && currentSortDir === 'asc'" width="14" height="14"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="18 15 12 9 6 15"></polyline>
                      </svg>
                      <svg v-else-if="currentSort === 'date' && currentSortDir === 'desc'" width="14" height="14"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="6 9 12 15 18 9"></polyline>
                      </svg>
                      <svg v-else width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ccc" stroke-width="2">
                ...
                <th class="clickable-header" @click="sort('status')">
                  <div class="d-flex align-items-center gap-1">
                    ...
                    <span class="sort-icon" :class="{ active: currentSort === 'status' }">
                      <svg v-if="currentSort === 'status' && currentSortDir === 'asc'" width="14" height="14"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="18 15 12 9 6 15"></polyline>
                      </svg>
                      <svg v-else-if="currentSort === 'status' && currentSortDir === 'desc'" width="14" height="14"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="6 9 12 15 18 9"></polyline>
                      </svg>
                      <svg v-else width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ccc" stroke-width="2">
            ...
            <tbody>
              <template v-for="project in paginatedProjects" :key="project.id">
                <tr class="main-row" :class="{ 'expanded': expandedRows.includes(project.id) }"
                  @click="toggleRow(project.id)">
                  <td class="ps-4">
                    <router-link :to="`/projekt/${project.id}`"
                  ...
                  <td>
                    <div :id="`hat-thumb-${project.id}`" class="img-wrapper"
                      style="width: 60px; height: 60px; padding: 5px">
                      <HatThumbnail :config="project.config" :patternsDict="patternsDict" class="d-flex" />
                    </div>
                  </td>
                  ...
                  <td>
                    <div @click.stop>
                      <StatusBadge :model-value="project.status" :project-id="project.id"
                        @statusChanged="handleStatusChange" />
                    </div>
                  </td>

                  <td class="text-end pe-4">
                    <button class="icon-btn me-2" title="Pobierz Kartę Produkcyjną"
                    ...
                    <button class="icon-btn transition-transform"
                        ...
                        <div class="col-md-3">
                          ...
                          <div class="d-flex flex-column gap-2 mt-3">
                            <div class="color-item">
                              ...
                              <span>Góra: <span class="text-muted font-monospace">{{ project.config.base.top
                              }}</span></span>
                            </div>
                            <div class="color-item">
                              <span class="color-dot" :style="{ background: project.config.base.middle }"></span>
                              <span>Środek: <span class="text-muted font-monospace">{{ project.config.base.middle
                              }}</span></span>
                            </div>
                            <div class="color-item">
                              <span class="color-dot" :style="{ background: project.config.base.bottom }"></span>
                              <span>Dół: <span class="text-muted font-monospace">{{ project.config.base.bottom
                              }}</span></span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="col-md-3">
                          ...
                          <div class="mt-3">
                            ...
                            <div class="d-flex gap-1 mb-3">
                              <div class="color-dot-sm" title="P1" :style="{ background: project.config.pompons.p1 }">
                              </div>
                              <div class="color-dot-sm" title="P2" :style="{ background: project.config.pompons.p2 }">
                              </div>
                              <div class="color-dot-sm" title="P3" :style="{ background: project.config.pompons.p3 }">
                              </div>
                              <div class="color-dot-sm" title="P4" :style="{ background: project.config.pompons.p4 }">
                              </div>
                            </div>
    ...
    <Teleport to="body">
      <div id="pdf-ghost-container" style="
        position: fixed; 
        left: -9999px; 
        top: 0; 
        width: 1200px; 
      ...
      ">
        <div v-if="pdfProjectData" style="
          width: 100%; 
          height: 100%; 
          visibility: visible; 
          background: transparent;
        ">
          <HatFlat :config="pdfProjectData.config" :patternsDict="patternsDict" style="width: 100%; height: 100%;" />
        </div>
      </div>
    </Teleport>
  </div>
...
import HatThumbnail from '../designer/hat/HatThumbnail.vue';
import { exportProjectsToCSV } from '../../utils/csvExport.js';
import { useProductionCard } from '../../utils/useProductionCard';
import StatusBadge from '../dashboard/admin/StatusBadge.vue';
...
.dashboard-wrapper {
  ...
  padding-bottom: 40px;
  /* Margines na dole, żeby tabela nie dotykała krawędzi */
}
...
.bg-blob {
  ...
  /* To pomaga w wydajności */
  pointer-events: none;
  /* Żeby nie blokowały kliknięć */
}
...
.custom-table {
  ...
  width: 100%;
  margin-bottom: 0;
  /* Usuń domyślny margines bootstrapa */
}
...
.table-responsive {
  ...
  overflow-x: auto;
  /* Tylko poziomy scroll na mobile */
  border-radius: 12px;
...
.main-row {
  ...
  /* Hack na wydajność renderowania wierszy */
}
...
@media (max-width: 600px) {
  ...
  .main-row td::before {
    ...
    /* Można użyć data-atrybutów lub sztywno w CSS */
    font-weight: 600;
    color: #9ca3af;
    font-size: 0.75rem;
    text-transform: uppercase;
    text-align: left;
    margin-right: auto;
    /* Popycha treść w prawo */
  }

  /* Definicje etykiet dla konkretnych kolumn */
  .main-row td:nth-of-type(1)::before {
    content: "ID / Data";
  }

  /* Miniaturka (2) nie ma etykiety */
  .main-row td:nth-of-type(3)::before {
    content: "Klient";
  }

  .main-row td:nth-of-type(4)::before {
    content: "Kontakt";
  }

  .main-row td:nth-of-type(5)::before {
    content: "Ilość";
  }

  .main-row td:nth-of-type(6)::before {
    content: "Status";
  }
  ...
  .main-row td:nth-of-type(2) {
    justify-content: center;
    /* Centrujemy miniaturkę */
    padding: 10px 0;
  }

  .main-row td:nth-of-type(2)::before {
    display: none;
  }
