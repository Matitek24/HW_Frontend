  ...
  <div class="color-picker-wrapper" ref="pickerTrigger">
    ...
    <Teleport to="body">
      <transition name="fade">
        <div v-if="isOpen" :class="isMobile ? 'color-dropdown-fixed' : 'color-dropdown-absolute'" :style="dropdownStyle"
          ref="dropdownEl" @click.stop>
          ...
          <div class="color-dropdown-grid" ref="gridEl">
            <div v-for="(color, index) in colorOptions" :key="color.id" class="color-dropdown-swatch"
              ...
              @mouseenter="(e) => handleSwatchHover(color, index, e)" @mouseleave="handleSwatchLeave"
              :class="{ active: modelValue === color.hex }">
            </div>
          </div>

          <div v-if="hoveredColor" class="floating-tooltip shared-tooltip" :class="{ 'is-bottom': tooltipIsBottom }"
...
const getActiveColorData = computed(() => {
  const index = props.colorOptions.findIndex(c => c.hex === props.modelValue);
  const activeColor = props.colorOptions[index];

  if (!activeColor) return { nazwa: props.modelValue, index: '' };

  return {
    nazwa: activeColor.nazwa,
    index: index + 1
  };
});
// --- CORE LOGIC ---
...
.color-ring:hover .main-trigger-tooltip {
  opacity: 1;
...
.color-dropdown-grid {
  ...
  gap: 6px;
  /* Mniejszy odstęp dla zwartości */
  max-height: 260px;
  overflow-y: auto;
  overflow-x: hidden;
  padding: 2px;
  /* Margines na focus ring */
}
...
.color-dropdown-swatch.active {
  ...
  /* Niebieski pierścień aktywności */
  z-index: 5;
}
