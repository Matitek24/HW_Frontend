<template>
    <div ref="canvasRef" class="export-canvas">
      
    
  
      <div class="export-logo">
        <img src="../assets/Headwear_COLOR_CMYK_logo-1.png.webp" width="180" />
      </div>
...
// WAŻNE: Musisz zaimportować bibliotekę wzorów, żeby dziecko wiedziało co to jest 'snowflakes'
import { PATTERN_LIBRARY } from '../utils/patterns.js'; 

// 1. Odbieramy dane. Nazwa musi być identyczna jak w rodzicu w :hatConfig="..."
...
// 2. ODTWARZAMY LOGIKĘ COMPUTED (To brakowało!)
// Dziecko otrzymało "hatConfig", ale musi samo sobie obliczyć HTML wzorów
const wzor_gora = computed(() => {
  return PATTERN_LIBRARY[props.hatConfig.patterns.top]?.svg || '';
});

const svgGraphic = computed(() => {
  return PATTERN_LIBRARY[props.hatConfig.patterns.bottom]?.svg || '';
});

const calculateTextPathOffset = computed(() => {
  return props.hatConfig.text.fontSize * 0.35;
});
...
const generateImage = async () => {
  ...
  try {
    ...
    link.download = `HEADWEAR-${Date.now()}.jpg`;
    link.href = dataUrl;
    link.click();
  } catch (err) {
    console.error('Błąd exportu:', err);
  }
};
...
defineExpose({ generateImage });
  ...
  <style scoped>
  /* --- STYL UKRYTEGO PŁÓTNA (EXPORT CANVAS) --- */
  ...
  z-index: -9999;
  visibility: hidden; /* ← Lepsze niż opacity */
  ...
  .export-content {
    ...
    gap: 50px;               /* Odstęp między czapkami */
    width: 100%;
    padding: 0 100px;
    margin-top: 40px;
  }
  ...
  .export-item {
    flex: 1; /* Obie zajmują tyle samo miejsca */
    display: flex;
    justify-content: center;
    max-height: 800px;
  }
  
  /* Jeśli SVG są za duże, skalujemy je wewnątrz exportu */
  .export-item svg {
    width: 100%;
    height: auto;
    filter: drop-shadow(0 20px 30px rgba(0,0,0,0.15)); /* Ładny cień pod czapką na obrazku */
  }
