<template>
  <div class="top-header">
    ...
    <div class="d-flex justify-content-center">
      ...
      <button class="glass-btn with-text me-3" :class="{ 'disabled-btn': isDownloading }" :disabled="isDownloading"
        ...
        <span class="btn-icon">
          <svg v-if="!isDownloading" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
          ...
          <svg v-else class="animate-spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <button class="glass-btn with-text" @click="handleMainAction"
        ...
        <span class="btn-icon">
          <svg v-if="isSubmitting" class="animate-spin" width="18" height="18" viewBox="0 0 24 24" fill="none"
          ...
          <svg v-else-if="!canEdit" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  <Transition name="fade">
    <div v-if="isLogoModalOpen" class="modal-overlay" @click.self="closeLogoModal">
      <div class="glass-modal-content container p-4" style="max-width: 400px;">
        ...
        <div class="text-center">
          ...
          <div v-else class="upload-area mb-4">
            <input type="file" id="logoInput" accept="image/png, image/jpeg" class="d-none" @change="handleLogoUpload">
            <label for="logoInput" class="upload-label">
              <svg class="mb-2" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#9ca3af"
                stroke-width="2">
      ...
      <div class="glass-modal-content container p-5 text-center" style="max-width: 450px;">
        ...
        <div class="mb-4 d-flex justify-content-center">
          <div class="p-3 rounded-circle bg-light shadow-sm">
            <svg v-if="isDownloading" class="animate-spin text-dark" width="48" height="48" viewBox="0 0 24 24"
              fill="#ffffff" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="12" y1="2" x2="12" y2="6"></line>
              <line x1="12" y1="18" x2="12" y2="22"></line>
              <line x1="4.93" y1="4.93" x2="7.76" y2="7.76"></line>
              <line x1="16.24" y1="16.24" x2="19.07" y2="19.07"></line>
            ...
            <svg v-else class="text-success" width="48" height="48" viewBox="0 0 24 24" fill="none"
...
@media (max-width: 600px) {
  .mobile-only {
    display: flex;
    /* Pokaż przycisk widoku */
  }

  .desktop-only {
    display: none;
    /* Opcjonalnie: Ukryj przycisk "Pobierz" na mobile, żeby zrobić miejsce */
  }
...
.action-btn-primary {
  background: #1f2937;
  /* Ciemny kolor z Twojego designu */
  color: white;
  border: none;
  border-radius: 12px;
  padding: 10px 28px;
...
.glass-modal-content {
  ...
  /* Półprzezroczyste białe */
  backdrop-filter: blur(20px);
  /* Mocny blur w samym oknie */
...
@media (max-width: 600px) {
  ...
  .glass-btn.with-text .btn-text {
    display: none;
  }

  /* Na mobilu ukryj tekst "Wyślij", zostaw ikonę */
  .glass-btn.with-text {
    padding: 0;
    width: 48px;
    border-radius: 50%;
...
@media (max-width: 600px) {
  ...
  .glass-modal-content {
    width: 90% !important;
    /* Zostaw margines po bokach */
    max-width: 90% !important;
    margin: 0 auto;
    /* Wyśrodkuj */
    padding: 20px !important;
    /* Mniejszy padding wewnątrz */
    max-height: 85vh;
    /* Żeby nie wyszedł poza ekran góra/dół */
    overflow-y: auto;
    /* Scrollowanie wewnątrz modala jak jest za wysoki */
  ...
  .input-pill {
    font-size: 16px;
    /* Zapobiega zoomowaniu na iOS przy kliknięciu w input */
    padding: 12px 16px;
  }
